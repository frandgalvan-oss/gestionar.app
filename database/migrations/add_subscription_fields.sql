-- Migración para agregar campos de suscripción y administrador
...
CREATE TABLE IF NOT EXISTS subscription_changes (
  ...
  change_type VARCHAR(50) NOT NULL, -- 'extend', 'reduce', 'cancel', 'grant_permanent', 'payment'
...
RETURNS TRIGGER AS $$
BEGIN
  -- Solo registrar si cambió el estado o la fecha de fin
...
BEGIN
  ...
  -- Si tiene premium permanente, siempre está activo
  IF profile_record.is_premium_permanent THEN
    RETURN TRUE;
  END IF;
  
  -- Si el estado es activo y la fecha no ha expirado
...
-- 13. Comentarios en las tablas para documentación
...
-- 14. Función para actualizar automáticamente el estado de suscripciones expiradas
...
-- 15. Crear un cron job para ejecutar la función diariamente (requiere extensión pg_cron)
-- Nota: Esto debe configurarse en Supabase Dashboard -> Database -> Cron Jobs
